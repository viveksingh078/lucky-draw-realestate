...
class PublicAccountController extends Controller
{
    ...
    public function getDashboard()
    {
        ...
        if ($user->isVendor()) {
            return $this->getVendorDashboard();
        }
     ...
     */
    protected function getVendorDashboard()
    {
        $user = auth('account')->user();
        
        // Get vendor properties stats
        $totalProperties = $user->properties()->count();
        $activeProperties = $user->properties()->where('moderation_status', 'approved')->count();
        $pendingProperties = $user->properties()->where('moderation_status', 'pending')->count();
        ...
        return view('plugins/real-estate::account.dashboard.vendor-dashboard', compact(
            'user',
            'totalProperties',
            'activeProperties',
            'pendingProperties',
