...
class LoginController extends Controller
{
    ...
    */
    ...
    public function showLoginForm(Request $request)
    {
        ...
        // Check if vendor login
        $isVendor = $request->get('type') === 'vendor';
        
        if ($isVendor) {
            SeoHelper::setTitle(__('Vendor Login'));
            
            if (view()->exists(Theme::getThemeNamespace() . '::views.real-estate.account.auth.login-vendor')) {
                return Theme::scope('real-estate.account.auth.login-vendor')->render();
            }
            
            return view('plugins/real-estate::account.auth.login-vendor');
        }
    ...
    protected function attemptLogin(Request $request)
    {
        if ($this->guard()->validate($this->credentials($request))) {
            ...
            // Check account type matches login type
            $expectedType = $request->get('account_type', 'user');
            if ($account->account_type !== $expectedType) {
                $loginType = $expectedType === 'vendor' ? 'vendor' : 'user';
                $accountType = $account->account_type === 'vendor' ? 'vendor' : 'user';
                
                throw ValidationException::withMessages([
                    'account_type' => [
                        __('This is a :account_type account. Please use :login_type login.', [
                            'account_type' => ucfirst($accountType),
                            'login_type' => ucfirst($loginType),
                        ]),
                    ],
                ]);
            }
            ...
            if ($account->account_status === 'pending') {
                throw ValidationException::withMessages([
                    'account_status' => [
                        __('Your account is under review. You will receive an email once your account is approved.'),
                    ],
                ]);
            }
            ...
            if ($account->account_status === 'rejected') {
                throw ValidationException::withMessages([
                    'account_status' => [
                        __('Your account has been rejected. Please contact support for more information.'),
                    ],
                ]);
            }
